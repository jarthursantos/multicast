...
import { formatDate, generateDateIntervals } from '~/utilities/date'
import { normalizeDate } from '~/utilities/normalizations'

import { IPurchaseResumesModel, IOptions } from './IPurchaseResumesModel'

interface IRawBuyerPurchaseResume
  extends Omit<IBuyerPurchaseResume, 'buyer' | 'representativity' | 'period'> {
  code: number
  name: string
}

interface IRawProviderPurchaseResume
  extends Omit<
    IProviderPurchaseResume,
...
type IRawEvolutionPurchaseResume = Omit<
  IEvolutionPurchaseResume,
  'representativity' | 'period' | 'weekDay'
>

type IRawUFPurchaseResume = Omit<
  IUFPurchaseResume,
  'representativity' | 'period'
>

interface IRawPurchaseResumeInvoice
  extends Omit<IPurchaseResumeInvoice, 'provider'> {
  providerCode: number
  providerName: string
...
export function createWinThorPurchaseResumesModel(): IPurchaseResumesModel {
  function formatCodes(codes: number[], field: string): string {
    return `(${codes.map(code => `${code} = ${field}`).join(' OR ')})`
  }

  return {
    async findPerBuyer(options: IOptions): Promise<IBuyerPurchaseResume[]> {
      const result: IBuyerPurchaseResume[] = []
      ...
      if (buyers.length !== 0) {
        baseParams += ` AND ${formatCodes(buyers, 'PCFORNEC.CODCOMPRADOR')}`
      }

      if (providers.length !== 0) {
        baseParams += ` AND ${formatCodes(providers, 'PCFORNEC.CODFORNEC')}`
      }
      ...
      `
        ...
        const response = await winthor.raw<IRawBuyerPurchaseResume[]>(`
    ...
    `
      ...
      const response = await winthor.raw<IRawPurchaseResumeInvoice[]>(`
    ...
    async findPerProvider(
      options: IOptions
    ): Promise<IProviderPurchaseResume[]> {
      const result: IProviderPurchaseResume[] = []
      ...
      if (buyers.length !== 0) {
        baseParams += ` AND ${formatCodes(buyers, 'MATRICULA')}`
      }

      if (providers.length !== 0) {
        baseParams += ` AND ${formatCodes(providers, 'B.CODFORNEC')}`
      }
      ...
      for (let i = 0; i < intervals.length; i++) {
        ...
        const response = await winthor.raw<IRawProviderPurchaseResume[]>(`
    ...
    async findPerEvolution(
      options: IOptions
    ): Promise<IEvolutionPurchaseResume[]> {
      const result: IEvolutionPurchaseResume[] = []
      ...
      `
        ...
        const response = await winthor.raw<IRawEvolutionPurchaseResume[]>(`
    ...
    async findPerUF(options: IOptions): Promise<IUFPurchaseResume[]> {
      const result: IUFPurchaseResume[] = []
      ...
      `
        ...
        const response = await winthor.raw<IRawUFPurchaseResume[]>(`
