...
import { IRevenuesRepository, Options } from 'repositories/IRevenuesRepository'
import {
  DateInterval,
  formatDate,
...
export class WinThorRevenuesRepository implements IRevenuesRepository {
  async findBySupervisor(options: Options): Promise<SupervisorRevenues[]> {
    ...
    const response = await winthor.raw<SupervisorRevenues[]>(`
  ...
  async findByMonth(options: Options): Promise<MonthRevenues[]> {
    let result: MonthRevenues[] = []
    ...
    for (let i = 0; i < intervals.length; i++) {
      ...
      const revenues = await this.findMonthRevenues(interval, options)
  ...
  private async findMonthRevenues(
    interval: DateInterval,
    options: Options
  ): Promise<MonthRevenues[]> {
    const { fromFormated, toFormated } = interval

    const params = this.parseOptions(options)

    const response = await winthor.raw<MonthRevenues[]>(`
      SELECT
  ...
  async findByDeadline(options: Options): Promise<DeadlineRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<DeadlineRevenues[]>(`
  ...
  async findByEvolution(options: Options): Promise<EvolutionRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const situationParams = this.parseOptions({
      clients: 'all',
      situation: options.situation
    })
    const clientsParams = this.parseOptions({
      clients: options.clients,
      situation: 'all'
    })

    const response = await winthor.raw<EvolutionRevenues[]>(`
  ...
  async findByClass(options: Options): Promise<ClassRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<ClassRevenues[]>(`
  ...
  async findByClassOrigin(options: Options): Promise<ClassRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)
  ...
  async findByClassPosition(options: Options): Promise<ClassRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)
  ...
  async findByClassSellType(options: Options): Promise<ClassRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)
  ...
  async findByRegion(options: Options): Promise<RegionRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<RegionRevenues[]>(`
  ...
  async findByEmitter(options: Options): Promise<EmitterRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<EmitterRevenues[]>(`
      SELECT PCPEDC.ORIGEMPED AS "origin",
  ...
  async findByAnalitic(options: Options): Promise<AnaliticRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<RawAnaliticRevenues[]>(`
  ...
  async findByCharge(options: Options): Promise<ChargeRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<ChargeRevenues[]>(`
  ...
  async findByCheckOut(options: Options): Promise<CheckOutRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<CheckOutRevenues[]>(`
  ...
  async findBySalesOrigin(options: Options): Promise<SalesOriginRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<SalesOriginRevenues[]>(`
      SELECT PCPEDC.ORIGEMPED AS "origin",
        COUNT(PCPEDC.NUMPED) AS "requestsCount",
        SUM(DECODE(PCPEDC.CONDVENDA, 5, 0, 6, 0, 11, 0, 12, 0, PCPEDC.VLATEND - NVL(PCPEDC.VLFRETE, 0) - NVL(PCPEDC.VLOUTRASDESP, 0))) AS "salesValue",
        SUM(NVL(NVL(DECODE(PCPEDC.CONDVENDA, 5, 0, 6, 0, 11, 0, 12, 0, PCPEDC.VLATEND - NVL(PCPEDC.VLFRETE, 0) - NVL(PCPEDC.VLOUTRASDESP, 0)), 0), 0)) - SUM(((SELECT SUM(NVL(I.VLCUSTOFIN, 0) * NVL(I.QT, 0)) FROM PCPEDI I WHERE I.NUMPED = PCPEDC.NUMPED AND I.DATA BETWEEN TO_DATE('${from}', 'DD/MM/YYYY') AND TO_DATE('${to}', 'DD/MM/YYYY')))) AS "profitValue",
  ...
  async findByActuationArea(
    options: Options
  ): Promise<ActuationAreaRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<ActuationAreaRevenues[]>(`
  ...
  async findByProvider(options: Options): Promise<ProviderRevenues[]> {
    const from = formatDate(normalizeDate(options.periodFrom))
    const to = formatDate(normalizeDate(options.periodTo))
    const params = this.parseOptions(options)

    const response = await winthor.raw<ProviderRevenues[]>(`
  ...
  private parseOptions({
    clients,
    situation
  }: Pick<Options, 'situation' | 'clients'>) {
    let params = ''
